...
class Solution:
    def goodNodes(self, root: TreeNode) -> int:
        ...
        while stack: # 1 element | 2 elements | 3 elements
            ...
            maxval = max(last_max, node.val) # 2, 2: 2| 2, 1: 2 | 2, 5: 5

            if node.right: # 1
                stack.append((node.right, maxval)) # [1,1] | [1 main_left, 1 right_left, 5]
             # [1 main_left, 1 right_left]
            if node.left:   # 1 | 1 | NONE
                stack.append((node.left, maxval)) # [1] | [1 main_left, 1 right_left] 
